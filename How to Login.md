...
2. **Required Configuration Files**
   - Create a new folder named `secrets/` in your project directory
   - Inside `secrets/`, create a `.env` file with your credentials:
     ```env
     KITE_API_KEY=your_api_key_here
     KITE_API_SECRET=your_api_secret_here
     ```
   - **Important**: The `config.py` script will access these credentials from `secrets/.env`
...
6. **Security Feature**: The first 5 digits of your access token will be displayed for verification
7. Tokens are saved to `secrets/.env_access` (separate from credentials file for better protection)
...
**Why separate files?**
- `secrets/.env` stores your permanent API credentials
- `secrets/.env_access` stores temporary access tokens that expire daily
- This separation provides better security - if tokens are compromised, your API keys remain safe
...
├── config.py                   # Configuration utilities (reads from secrets/)
└── secrets/                    # Create this folder manually
    ├── .env                    # Your API credentials (create manually)
    │                           # Contains: KITE_API_KEY, KITE_API_SECRET
    └── .env_access             # Access tokens (auto-generated by get_token_local.py)
                                # Contains: KITE_ACCESS_TOKEN, KITE_REQUEST_TOKEN, KITE_TOKEN_SAVED_TIME
```

**Setup Steps:**
1. Create `secrets/` folder in your `pykite` directory
2. Create `secrets/.env` file with your API credentials
3. Run `get_token_local.py` - it will create `secrets/.env_access` automatically

## Security Notes

🔒 **Keep these files private:**
- `secrets/.env` - Contains your permanent API key and secret
- `secrets/.env_access` - Contains your temporary access token (expires daily)
...
🛡️ **Security Design:**
- **Token Preview**: Only the first 5 characters of your access token are printed during login for security
- **File Separation**: API credentials and access tokens are stored in separate files
- **Token Expiry**: Access tokens expire daily, limiting exposure if compromised
- **Timestamp Tracking**: Token save time is recorded to help identify stale tokens
